...
int main(int argc, char *argv[]) {
  ...
  // Write to the server
  charsWritten = send(socketFD, "I am dec server", 16, 0);
  charsRead = recv(socketFD, verify, 499, 0);
  if (strcmp(verify, "Connection refused") == 0) {
  ...
  // * msg format: <key>#<ciphertext>$
  // * allocating space for key + '#' + ciphertext + '$' + '\0'
